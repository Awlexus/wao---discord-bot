...
var _custom_emojis = require('../custom_emojis');

var _custom_emojis2 = _interopRequireDefault(_custom_emojis);

function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }

var prefix = process.env.BOT_PREFIX;
var emoticons = _emoticons2.default['emoticons'].concat(_custom_emojis2.default);
...
function randomEmoticonWithTag(searchTag) {
  var rand = function rand(arr) {
    return arr[Math.floor(Math.random() * arr.length)];
  };

  searchTag = searchTag.trim().toLowerCase();
  if (!searchTag) return rand(emoticons);
  var withTag = emoticons.filter(function (emoticon) {
...
exports.default = {
  ...
  fromBot: fromBot,
  splitOnFirstWord: splitOnFirstWord
};